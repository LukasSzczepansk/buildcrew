...
const SEO = {
  pl: {
    ...
    description: "Znajdź ludzi do projektu, dołącz do zespołu i buduj portfolio oraz sieć kontaktów przez realną współpracę.",
  },
  en: {
    ...
    <div className="min-h-screen bg-[#f4f4ef] text-[#111111] dark:bg-[#11110f] dark:text-[#f4f4ef]">
      ...
      <header className="border-b border-[#d8d8d0] dark:border-[#34342f]">
        <div className="mx-auto flex h-16 max-w-[1240px] items-center justify-between px-5 sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Button asChild size="sm"><Link href="/signup">{c("Załóż profil", "Create profile")}</Link></Button>
          </div>
        </div>
      </header>
      ...
      <main>
        <section className="mx-auto grid max-w-[1240px] gap-12 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-[minmax(0,1fr)_430px] lg:items-end lg:px-10 lg:py-24">
          <div>
            <p className="text-[13px] font-medium text-neutral-500 dark:text-neutral-400">{c("Ludzie · projekty · współpraca · możliwości", "People · projects · proof of work · opportunities")}</p>
            ...
            <p className="mt-6 max-w-2xl text-[16px] leading-7 text-neutral-600 dark:text-neutral-300">{c("Znajduj ludzi do projektu, dołączaj do zespołów i pokazuj, co naprawdę budujesz. BuildCrew pomaga zamienić wspólną pracę w portfolio, kontakty i kolejne możliwości.", "Find teammates, co-founders and collaborators. Show what you are building, grow a professional network around real work and make it easier for the right people to discover you.")}</p>
            <div className="mt-7 flex flex-wrap gap-2">
              <Button asChild size="lg"><Link href="/signup">{c("Załóż profil", "Create your profile")} <ArrowRight className="h-3.5 w-3.5" /></Link></Button>
        ...
        <section id="people" className="border-y border-[#d8d8d0] bg-white dark:border-[#34342f] dark:bg-[#171715]">
          <div className="mx-auto max-w-[1240px] px-5 py-12 sm:px-8 lg:px-10">
            <div className="mb-7 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
              <div><p className="text-[13px] font-medium text-neutral-500">{c("Sieć BuildCrew", "BuildCrew network")}</p><h2 className="mt-1 text-[28px] font-semibold tracking-[-0.025em]">{c("Ludzie otwarci na współpracę", "People open to opportunities")}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500 dark:text-neutral-400">{c("Znajduj osoby po umiejętnościach, projektach, dostępności i tym, czego aktualnie szukają.", "Discover people by skills, projects, availability and what they are open to next.")}</p></div>
              <Link href="/signup" className="text-sm font-medium hover:underline">{c("Dołącz do sieci", "Join the network")} <ArrowRight className="inline h-3.5 w-3.5" /></Link>
            </div>
        ...
        <section className="mx-auto max-w-[1240px] px-5 py-16 sm:px-8 sm:py-20 lg:px-10">
          <div className="grid gap-10 lg:grid-cols-[280px_minmax(0,1fr)]">
            ...
            <div className="border-t border-[#b9b9b1] dark:border-neutral-600">
              ...
              <ValueRow number="02" title={c("Buduj i pokazuj projekty", "Build and showcase projects")} text={c("Projekt staje się dowodem tego, co potrafisz stworzyć, a nie tylko kolejnym punktem w profilu.", "Projects become proof of what you can create, not just another line in a profile.")} />
              <ValueRow number="03" title={c("Rozwijaj sieć kontaktów", "Grow your professional network")} text={c("Kontakt ma większą wartość, gdy widać, co dana osoba zbudowała i z kim pracowała.", "Connections become more meaningful when you can see what someone built and who they worked with.")} />
              <ValueRow number="04" title={c("Daj się znaleźć", "Get discovered for work")} text={c("Zaznacz, że jesteś otwarty na pracę, freelance albo staż i pozwól, żeby Twoje projekty mówiły za Ciebie.", "Mark yourself open to full-time, freelance or internship opportunities and let your work speak before a traditional recruiting process starts.")} />
            </div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="border-y border-[#d8d8d0] bg-white dark:border-[#34342f] dark:bg-[#171715]">
          <div className="mx-auto grid max-w-[1240px] gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[280px_minmax(0,1fr)] lg:px-10">
            <div><p className="text-[13px] font-medium text-neutral-500">{c("Jak to działa", "How it works")}</p><h2 className="mt-2 text-[28px] font-semibold tracking-[-0.025em]">{c("Profil → ludzie → projekty → reputacja.", "Profile → people → projects → reputation.")}</h2></div>
            <ol className="border-t border-[#b9b9b1] dark:border-neutral-600">
              <ValueRow number="1" title={c("Uzupełnij profil", "Create your builder profile")} text={c("Dodaj umiejętności, zainteresowania, dostępność, linki i to, na jakie możliwości jesteś otwarty.", "Add skills, interests, availability, links and the opportunities you are open to.")} />
              ...
              <ValueRow number="3" title={c("Zbudujcie coś razem", "Build something together")} text={c("Dołącz do projektu albo zaproś kogoś do swojego zespołu i zachowaj historię wspólnej pracy.", "Join a project, invite someone to yours and keep your project history connected to the people behind it.")} />
              <ValueRow number="4" title={c("Zamień pracę w reputację", "Turn work into reputation")} text={c("Ukończone projekty i historia współpracy wzmacniają Twój profil przed kolejnym projektem, rozmową z co-founderem albo ofertą pracy.", "Completed projects and collaboration history make your profile stronger for the next project, co-founder conversation or job opportunity.")} />
            </ol>
          </div>
        </section>

        <section className="border-y border-neutral-800 bg-[#151513] text-neutral-100"><div className="mx-auto grid max-w-[1240px] gap-8 px-5 py-14 sm:px-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end lg:px-10"><div><p className="text-[13px] text-neutral-400">BuildCrew</p><h2 className="mt-3 max-w-3xl text-[34px] font-semibold leading-[1.15] tracking-[-0.03em] sm:text-[42px]">{c("Pokaż, co budujesz. Poznaj ludzi, z którymi warto budować. Twórz możliwości przez realną pracę.", "Show what you build. Meet people worth building with. Create opportunities from real work.")}</h2></div><Button asChild variant="secondary" size="lg"><Link href="/signup">{c("Załóż profil", "Create profile")}</Link></Button></div></section>
      </main>
