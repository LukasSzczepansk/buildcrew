      ...
      <main>
        <section className="border-b border-[#d8d8d0] dark:border-[#34342f]">
          <div className="mx-auto grid max-w-[1240px] gap-10 px-5 py-14 sm:px-8 sm:py-18 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-end lg:px-10 lg:py-20">
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-medium text-neutral-500 dark:text-neutral-400"><span>{c("Zgłoszenie zajmuje około 2-3 minut.", "The application takes about 2-3 minutes.")}</span><span className="hidden h-1 w-1 rounded-full bg-neutral-300 sm:inline-block dark:bg-neutral-600" /><Link href="/sprint/regulamin" className="underline decoration-neutral-300 underline-offset-4 hover:text-neutral-900 dark:decoration-neutral-600 dark:hover:text-white">{c("Regulamin Sprintu", "Sprint Terms")}</Link></div>
            ...
            <Card className="overflow-hidden bg-white dark:bg-[#171715]">
              ...
              <div className="grid grid-cols-2 gap-px bg-[var(--bc-line)]"><Stat icon={CalendarDays} label={c("Start", "Start")} value={start} /><Stat icon={Users} label={c("Zgłoszenia", "Applications")} value={`${stats.total}/${capacity}`} /><Stat icon={Sparkles} label="Team Reveal" value={teamReveal} /><Stat icon={Rocket} label="Demo Day" value={demoDay} /></div>
              <div className="p-5">
                <div className="flex items-center justify-between text-xs"><span className="font-medium">{c("Miejsca", "Seats")}</span><span className="text-neutral-500">{remaining > 0 ? c(`${remaining} wolnych`, `${remaining} left`) : c("Lista pełna", "Full")}</span></div>
        ...
        <section className="mx-auto max-w-[1240px] px-5 py-12 sm:px-8 lg:px-10">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <RoleCard icon={Palette} title="UX/UI" text={c("Projektujesz dobre rzeczy, ale brakuje Ci developerów, którzy je wdrożą? Tu ich znajdziesz.", "Design good products but lack developers to implement them? Find them here.")} count={stats.roleCounts.UI_UX ?? 0} countLabel={c("zgłoszeń", "applications")} />
            <RoleCard icon={Lightbulb} title={c("Product / Founder", "Product / Founder")} text={c("Masz pomysł, ale nie masz kompletnej ekipy? Sprint pomoże domknąć brakujące role.", "Have an idea but not a complete team? Sprint helps fill the missing roles.")} count={stats.roleCounts.PRODUCT ?? 0} countLabel={c("zgłoszeń", "applications")} />
            <RoleCard icon={TrendingUp} title="Growth / Marketing" text={c("Chcesz pracować przy produkcie, który naprawdę powstaje, a nie przy fikcyjnym case study.", "Work on a product that is actually being built, not a fictional case study.")} count={stats.roleCounts.MARKETING ?? 0} countLabel={c("zgłoszeń", "applications")} />
          </div>
        </section>
...
function RoleCard({ icon: Icon, title, text, count, countLabel }: { icon: LucideIcon; title: string; text: string; count: number; countLabel: string }) { return <Card className="p-5"><div className="flex items-center justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#eef6d6] text-[#66820f] dark:bg-lime-400/10 dark:text-lime-300"><Icon className="h-5 w-5" /></span><span className="text-xs font-medium text-neutral-400">{count} {countLabel}</span></div><h3 className="mt-5 text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-neutral-500 dark:text-neutral-400">{text}</p></Card>; }
