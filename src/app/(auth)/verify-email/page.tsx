...
export default async function VerifyEmailPage({
  ...
  const [{ token, sent, next }, locale] = await Promise.all([searchParams, getRequestLocale()]);
  const en = locale === "en";
  ...
  if (token) {
    return (
      <div>
        ...
        <h1 className="mt-2 text-[26px] font-semibold tracking-[-0.025em] text-[var(--bc-ink)]">{en ? "Confirming your email" : "Potwierdzamy Twój e-mail"}</h1>
