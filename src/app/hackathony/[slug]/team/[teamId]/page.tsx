...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; teamId: string }> }): Promise<Metadata> {
  ...
  if (!event) return { title: locale === "en" ? "Hackathon team - BuildCrew" : "Team hackathonowy - BuildCrew", robots: { index: false, follow: false } };
  ...
  const title = locale === "en" ? `${team.name} is looking for teammates for ${event.name} | BuildCrew` : `${team.name} szuka teamu na ${event.name} | BuildCrew`;
  const description = locale === "en" ? `${team.members.length}/${team.targetSize} people in the team. See missing roles and join through BuildCrew.` : `${team.members.length}/${team.targetSize} osób w teamie. Zobacz brakujące role i dołącz przez BuildCrew.`;
...
export default async function PublicHackathonTeamPage({ params }: { params: Promise<{ slug: string; teamId: string }> }) {
  ...
  return <div className="min-h-screen bg-[var(--bc-canvas)] text-[var(--bc-ink)]">
    <header className="border-b border-[var(--bc-line)]"><div className="mx-auto flex h-16 max-w-[1180px] items-center justify-between px-5 sm:px-8 lg:px-10"><Link href="/" className="flex items-center gap-2 text-[17px] font-semibold tracking-[-0.02em]"><span className="h-4 w-[5px] bg-[var(--bc-accent)]" />BuildCrew</Link><Button asChild size="sm"><Link href={`/signup?next=/hackathons/${event.slug}`}>{copy("Dołącz do team findera", "Join team finder")}</Link></Button></div></header>
