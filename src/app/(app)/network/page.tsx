  ...
  return (
    <div>
      ...
      <section className="grid border-y border-[var(--bc-line)] sm:grid-cols-4">
        ...
        <Metric value={counts.followers} label={en ? "followers" : "obserwujących"} />
      ...
      <nav className="mt-6 flex gap-1 overflow-x-auto border-b border-[var(--bc-line)]" aria-label={en ? "My network" : "Moja sieć"}>
        ...
        <NetworkTab active={tab === "following"} href="/network?tab=following">{en ? "Following" : "Obserwowani"} <span>{following.length}</span></NetworkTab>
        <NetworkTab active={tab === "followers"} href="/network?tab=followers">{en ? "Followers" : "Obserwujący"} <span>{followers.length}</span></NetworkTab>
      ...
      <div className="mt-6 grid gap-9 xl:grid-cols-[minmax(0,1fr)_320px]">
        <main className="min-w-0">
          {tab === "collaborators" ? (
            <NetworkSection title={en ? "People you actually built with" : "Ludzie, z którymi naprawdę coś zbudowałeś"} description={en ? "This relationship is verified automatically when you are members of the same project." : "Ta relacja powstaje automatycznie, gdy jesteście członkami tego samego projektu."}>
              {collaborators.length ? collaborators.map((item) => (
                <PersonRow locale={locale} key={item.profile.userId} profile={item.profile} meta={`${item.sharedProjects} ${en ? (item.sharedProjects === 1 ? "shared project" : "shared projects") : (item.sharedProjects === 1 ? "wspólny projekt" : "wspólne projekty")}${item.latestProject ? ` · ${en ? "latest" : "ostatni"} ${item.latestProject.name}` : ""}`} openToBuild={isOpenToOpportunities(item.profile.lookingFor)}>
              ...
              )) : <EmptyNetwork title={en ? "No collaboration history yet" : "Nie masz jeszcze historii współpracy"} text={en ? "Join a project or create your own team. Shared projects will automatically start building your network." : "Dołącz do projektu albo stwórz własny zespół. Wspólne projekty automatycznie zaczną budować Twoją sieć."} href="/projects" cta={en ? "Find a project" : "Znajdź projekt"} />}
            </NetworkSection>
          ) : null}

          {tab === "following" ? (
            <NetworkSection title={en ? "Following" : "Obserwowani"} description={en ? "Following does not require approval. You will get a signal when this person publishes a new project." : "Obserwowanie nie wymaga akceptacji. Dostaniesz sygnał, gdy ta osoba opublikuje nowy projekt."}>
              {following.length ? following.map((item) => (
                <PersonRow locale={locale} key={item.profile.userId} profile={item.profile} meta={`${en ? "Following since" : "Obserwujesz od"} ${timeAgo(item.since, locale)}`} openToBuild={isOpenToOpportunities(item.profile.lookingFor)}>
          ...
          {tab === "followers" ? (
            <NetworkSection title={en ? "Followers" : "Obserwujący"} description={en ? "People who want to see your new projects and collaboration availability." : "Osoby, które chcą widzieć Twoje nowe projekty i dostępność do współpracy."}>
              {followers.length ? followers.map((item) => (
                <PersonRow locale={locale} key={item.profile.userId} profile={item.profile} meta={`${en ? "Following you since" : "Obserwuje Cię od"} ${timeAgo(item.since, locale)}`} openToBuild={isOpenToOpportunities(item.profile.lookingFor)}>
                  <FollowButton targetUserId={item.profile.userId} initialFollowing={followingIds.has(item.profile.userId)} compact />
                  <Button asChild variant="outline" size="sm"><Link href={`/builders/${item.profile.userId}`}>{en ? "Profile" : "Profil"}</Link></Button>
                </PersonRow>
              )) : <EmptyNetwork title={en ? "No followers yet" : "Brak obserwujących"} text={en ? "Complete your profile, say what you are looking for and take part in projects. Your network should grow from real activity, not random invites." : "Uzupełnij profil, zaznacz czego szukasz i bierz udział w projektach. Sieć powinna rosnąć z realnej aktywności, a nie losowych zaproszeń."} href="/profile" cta={en ? "Complete profile" : "Uzupełnij profil"} />}
            </NetworkSection>
          ) : null}
            ...
            <div className="space-y-8">
              <NetworkSection title={en ? "Connections" : "Kontakty"} description={en ? "Accepted connections can message each other privately on BuildCrew." : "Zaakceptowane kontakty mogą pisać do siebie prywatnie w BuildCrew."}>
                {friends.length ? friends.map((item) => (
                  <PersonRow locale={locale} key={item.friendshipId} profile={item.profile} meta={`${en ? "Connected since" : "Kontakt od"} ${timeAgo(item.since, locale)}`} openToBuild={isOpenToOpportunities(item.profile.lookingFor)}>
                    <FriendRelationActions targetUserId={item.profile.userId} state={{ kind: "FRIENDS", conversationId: item.conversationId }} compact />
                  </PersonRow>
                )) : <EmptyNetwork title={en ? "No accepted connections yet" : "Brak zaakceptowanych kontaktów"} text={en ? "Connections are useful when you want to talk one-on-one. Follow people when you only want to keep up with them." : "Kontakty są przydatne, gdy chcesz rozmawiać 1:1. Obserwuj osoby, jeśli chcesz tylko śledzić ich aktywność."} href="/builders" cta={en ? "Find people" : "Znajdź ludzi"} />}
              </NetworkSection>
    ...
    <div className="grid gap-3 py-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
      <Link href={`/builders/${profile.userId}`} className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-semibold">{profile.username}</p>{openToBuild ? <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[var(--bc-muted)]"><span className="h-1.5 w-1.5 rounded-full bg-[var(--bc-accent-strong)]" />{locale === "en" ? "Open to opportunities" : "Otwarty na możliwości"}</span> : null}</div>
