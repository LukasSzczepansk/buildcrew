...
export default async function HelpPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex justify-end"><Button asChild size="sm"><Link href="/help/new"><Plus className="h-4 w-4" /> Zadaj pytanie</Link></Button></div>

      {questions.length === 0 ? (
        <Card className="p-12 text-center">
          <MessagesSquare className="mx-auto h-8 w-8 text-neutral-300" />
          <h2 className="mt-4 font-semibold">Nie ma jeszcze pytań</h2>
          <p className="mt-1 text-sm text-[var(--bc-muted)]">Możesz być pierwszą osobą, która o coś zapyta.</p>
