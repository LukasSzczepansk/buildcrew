  ...
  return (
    <div>
      ...
      <nav className="flex gap-6 overflow-x-auto border-b border-[var(--bc-line)]" aria-label={en ? "Project management" : "Zarządzanie projektem"}>
        ...
        <ManageTab href={`/projects/${id}/workspace`} label={en ? "Workspace" : "Przestrzeń pracy"} />
      </nav>
      ...
      <div className="mt-7 grid gap-9 lg:grid-cols-[minmax(0,1fr)_280px]">
        <main>
          <div className="mb-4">
            ...
            <p className="mt-1 max-w-[680px] text-[13px] leading-5 text-[var(--bc-muted)]">{en ? "Only the project owner can remove team members. Removed members lose access to the private workspace." : "Tylko właściciel projektu może usuwać członków zespołu. Usunięta osoba traci dostęp do prywatnej przestrzeni pracy."}</p>
          </div>
          <ProjectTeamManager
            projectId={id}
        ...
        <aside className="space-y-6">
          <section className="border-b border-[var(--bc-line)] pb-6">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--bc-faint)]">{en ? "Discovery settings" : "Ustawienia odkrywania"}</p>
            <p className="mt-2 text-[12px] leading-4 text-[var(--bc-muted)]">{en ? "Set reach, location and current needs so the right people can discover the project." : "Ustaw zasięg, lokalizację i aktualne potrzeby, aby właściwe osoby mogły znaleźć projekt."}</p>
          ...
          <section>
            ...
            <p className="mt-2 text-[12px] leading-4 text-[var(--bc-muted)]">{en ? "Pause an inactive project or complete it when the team has shipped a result. Completion saves contributor credits." : "Wstrzymaj nieaktywny projekt albo oznacz go jako ukończony, gdy zespół dowiezie rezultat. Ukończenie zapisuje wkład współtwórców."}</p>
          ...
          <section>
            ...
            <div className="mt-2 flex flex-col items-start gap-1">
              <Link href={`/projects/${id}/applications`} className="text-[13px] text-[var(--bc-muted)] hover:text-[var(--bc-ink)] hover:underline">{en ? "Open applications" : "Otwórz zgłoszenia"}</Link>
