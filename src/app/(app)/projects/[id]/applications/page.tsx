...
export default async function ProjectApplicationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><Topbar title={en ? `Applications - ${project.name}` : `Zgłoszenia - ${project.name}`} subtitle={en ? `${pendingCount} ${pendingCount === 1 ? "person is" : "people are"} waiting for a decision. See what they could bring to the team.` : `${pendingCount} osób czeka na decyzję. Zobacz, co mogą wnieść do ekipy.`} />
    {applications.length === 0 ? <EmptyState icon="📭" title={en ? "No applications yet." : "Jeszcze nikt się nie zgłosił."} description={en ? "You can actively find builders and invite them to your project." : "Możesz aktywnie znaleźć builderów i zaprosić ich do projektu."} ctaLabel={en ? "Find builders" : "Znajdź builderów"} ctaHref="/builders" /> : <div className="flex flex-col gap-4">{applications.map((a) => { const applicantSkills = skillsByApplicant.get(a.applicant.userId) ?? []; const targetSkills = a.role.skills.length ? a.role.skills : project.technologies; const overlap = applicantSkills.filter((skill) => targetSkills.includes(skill)); let matchScore = Math.min(30, overlap.length * 10); const reasons: string[] = []; if (overlap.length) reasons.push(en ? `Matching skills: ${overlap.slice(0, 3).join(", ")}` : `Pasujące umiejętności: ${overlap.slice(0, 3).join(", ")}`); if (a.applicant.role === a.role.roleType || a.applicant.role === "FULLSTACK") { matchScore += 40; reasons.push(en ? "Role matches the open position" : "Rola pasuje do otwartego miejsca"); } if (a.role.preferredLevel && a.applicant.level === a.role.preferredLevel) { matchScore += 15; reasons.push(en ? "Experience level matches" : "Pasuje poziom doświadczenia"); } if (project.commitment && a.applicant.weeklyHours === project.commitment) { matchScore += 15; reasons.push(en ? "Similar availability" : "Podobna dostępność"); } return <ApplicationCard key={a.id} application={{ id: a.id, status: a.status, message: a.message, role: { roleType: a.role.roleType as RoleType }, matchScore: Math.min(100, matchScore), reasons, applicant: { userId: a.applicant.userId, username: a.applicant.username, avatarEmoji: a.applicant.avatarEmoji, level: a.applicant.level, weeklyHours: a.applicant.weeklyHours, skills: applicantSkills } }} />; })}</div>}
  </div>;
}
