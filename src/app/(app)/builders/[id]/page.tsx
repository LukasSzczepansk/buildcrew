...
export default async function BuilderProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const projects = [...ownedProjects.map((p) => ({ ...p, relation: en ? "Owner" : "Właściciel" })), ...memberProjects.filter((p) => p.ownerId !== id).map((p) => ({ ...p, relation: en ? "Team member" : "Członek zespołu" }))];
        ...
        <main className="min-w-0">
          <section className="border-b border-[var(--bc-line)] pb-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0">
                ...
                {(profile.lookingFor.includes("OPEN_TO_BUILD") || profile.lookingFor.includes("WANTS_PROJECT")) ? <div className="mt-2 inline-flex items-center gap-2 text-[12px] font-medium text-[var(--bc-ink)]"><span className="h-2 w-2 rounded-full bg-[var(--bc-accent-strong)]" />{en ? "Open to build · open to a new project" : "Open to build · otwarty na nowy projekt"}</div> : null}
          ...
          <ProfileSection title={en ? "Availability and direction" : "Dostępność i kierunek"}>
            <div className="grid gap-4 text-sm sm:grid-cols-2">
              ...
              <div><p className="text-[12px] text-[var(--bc-faint)]">{en ? "Looking for" : "Szukam teraz"}</p><p className="mt-1 font-medium">{profile.lookingFor.length ? profile.lookingFor.map((item) => labels.lookingFor[item]).join(" · ") : "-"}</p></div>
          ...
          {(endorsementSummary.total > 0 || networkCounts.collaborators > 0) ? (
            <ProfileSection title={en ? "Collaboration history" : "Historia współpracy"}>
              <div className="grid gap-5 sm:grid-cols-3">
                ...
                <div><p className="text-[20px] font-semibold tabular-nums">{endorsementSummary.wouldAgain}</p><p className="mt-0.5 text-[11px] uppercase tracking-[0.08em] text-[var(--bc-faint)]">{en ? "would work again" : "chce pracować ponownie"}</p></div>
              </div>
