...
export default async function ShowcasePage({ searchParams }: { searchParams: Promise<{ tab?: string; category?: string }> }) {
  ...
  return (
    <div>
      <Topbar title="Showcase" subtitle="Projekty i eksperymenty opublikowane przez społeczność." />
      ...
      {entries.some((entry) => entry.isDemo) ? <div className="mb-5 rounded-[8px] border border-[var(--bc-line)] bg-[var(--bc-surface-subtle)] px-4 py-3 text-xs text-[var(--bc-muted)]"><span className="font-semibold text-[var(--bc-ink)]">Demo</span> oznacza przykładową zawartość.</div> : null}
