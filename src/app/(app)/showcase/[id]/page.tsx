  ...
  return <div><Topbar title={entry.title} subtitle={entry.tagline} />
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <div className="space-y-6">
        ...
        <Card className="p-6"><div className="flex items-center gap-2"><MessageSquareText className="h-5 w-5 text-violet-600" /><h2 className="text-lg font-semibold">Konstruktywny feedback</h2></div>{user && user.id !== entry.creatorId ? <div className="mt-5"><ShowcaseFeedbackForm entryId={entry.id} /></div> : <p className="mt-3 text-sm text-neutral-500">{user ? "To Twój projekt — tutaj zobaczysz opinie społeczności." : "Zaloguj się, żeby dać feedback."}</p>}</Card>
        {feedback.length ? <div className="space-y-3">{feedback.map((item) => <Card key={item.id} className="p-5"><div className="flex items-center gap-3"><Avatar emoji={item.avatarEmoji} size="sm" /><div><Link href={`/builders/${item.userId}`} className="text-sm font-semibold hover:underline">{item.username}</Link><p className="text-xs text-neutral-400">Czy używałby: {item.wouldUse === "YES" ? "Tak" : item.wouldUse === "MAYBE" ? "Może" : "Nie"}</p></div></div>{item.liked ? <p className="mt-3 text-sm"><span className="font-medium">👍 Co działa:</span> {item.liked}</p> : null}{item.improve ? <p className="mt-2 text-sm"><span className="font-medium">🛠 Co poprawić:</span> {item.improve}</p> : null}</Card>)}</div> : null}
      </div>
      <div className="space-y-5">
        <Card className="p-5"><h3 className="flex items-center gap-2 font-semibold"><Users className="h-4 w-4" /> Twórcy</h3><div className="mt-4 space-y-3">{entry.team.map((member) => <Link key={member.userId} href={`/builders/${member.userId}`} className="flex items-center gap-3"><Avatar emoji={member.avatarEmoji} size="sm" /><span className="text-sm font-medium">{member.username}</span></Link>)}</div></Card>
        {entry.lookingForCollaborators ? <Card className="border-violet-200 bg-violet-50/60 p-5 dark:border-violet-500/20 dark:bg-violet-500/5"><h3 className="font-semibold text-violet-800 dark:text-violet-300">Ekipa nadal rośnie</h3><p className="mt-2 text-sm text-violet-700/80 dark:text-violet-300/80">{entry.lookingForText || "Twórcy są otwarci na kolejnych współtwórców."}</p>{entry.projectId ? <Button asChild className="mt-4 w-full"><Link href={`/projects/${entry.projectId}`}>Chcę dołączyć</Link></Button> : entry.creator ? <Button asChild className="mt-4 w-full"><Link href={`/builders/${entry.creator.userId}`}>Poznaj twórcę</Link></Button> : null}</Card> : null}
