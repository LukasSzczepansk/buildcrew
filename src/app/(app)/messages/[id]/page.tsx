...
export async function generateMetadata(): Promise<Metadata> { const locale = await getRequestLocale(); return { title: locale === "en" ? "Conversation - BuildCrew" : "Rozmowa - BuildCrew" }; }
export default async function ConversationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-4xl"><Button asChild variant="ghost" size="sm" className="mb-4 gap-1.5"><Link href="/messages"><ChevronLeft className="h-4 w-4" /> {en ? "All messages" : "Wszystkie wiadomości"}</Link></Button><ChatThread conversationId={id} currentUserId={user.id} otherUser={{ username: access.profile.username, avatarEmoji: access.profile.avatarEmoji }} initialMessages={items.map((message) => ({ id: message.id, senderId: message.senderId, body: message.body, createdAt: message.createdAt.toISOString(), readAt: message.readAt?.toISOString() ?? null }))} /></div>;
}
