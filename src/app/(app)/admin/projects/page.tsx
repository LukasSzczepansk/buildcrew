          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
              {rows.map((project) => <tr key={project.id} className="hover:bg-neutral-50/60 dark:hover:bg-neutral-800/30">
                ...
                <td className="px-5 py-4"><div className="flex justify-end gap-2"><Link href={`/projects/${project.id}`} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-neutral-200 px-3 text-xs font-medium hover:bg-neutral-50 dark:border-neutral-700 dark:hover:bg-neutral-800"><ExternalLink className="h-3.5 w-3.5"/> Otwórz</Link><form action={deleteProjectAdminAction}><input type="hidden" name="projectId" value={project.id}/><ConfirmSubmit message={`Usunąć projekt „${project.name}”? Tej operacji nie można cofnąć.`} variant="destructive" size="sm"><Trash2 className="h-3.5 w-3.5"/> Usuń</ConfirmSubmit></form></div></td>
              </tr>)}
            </tbody>
          </table>
        </div>
