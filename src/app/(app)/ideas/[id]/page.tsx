      ...
      <main className="mx-auto max-w-[940px]">
        <section className="border-y border-[var(--bc-line)] py-7">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0 max-w-[720px]">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--bc-faint)]">Pomysł · {idea.interestedCount} zainteresowanych</p>
              ...
              <div className="mt-5 flex flex-wrap gap-x-2 gap-y-1 text-[12px] text-[var(--bc-muted)]">{idea.interests.map((interest) => <span key={interest}>{interest}</span>)}</div>
            </div>
        ...
        <section className="grid gap-8 py-7 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div>
            ...
            <Link href={`/builders/${idea.ownerId}`} className="mt-3 flex max-w-[420px] items-center gap-3 border-y border-[var(--bc-line)] py-4 hover:bg-[var(--bc-surface-subtle)]">
              ...
              <div><p className="text-[14px] font-medium">{idea.owner?.username ?? "Builder"}</p><p className="text-[11px] text-[var(--bc-muted)]">Zobacz profil →</p></div>
            </Link>
          </div>

          <aside className="border-l-0 border-[var(--bc-line)] lg:border-l lg:pl-6">
            <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-[var(--bc-faint)]">Co dalej</p>
            <p className="mt-2 text-[12px] leading-5 text-[var(--bc-muted)]">Jeśli kierunek Cię interesuje, zaznacz to. Autor zobaczy Twój profil i może zacząć rozmowę.</p>
        ...
        {owner ? (
          <section className="border-t border-[var(--bc-line)] pt-7">
            <div className="flex items-end justify-between gap-4"><h2 className="text-[18px] font-semibold">Kto jest zainteresowany</h2><span className="text-[12px] text-[var(--bc-faint)]">{idea.interestedProfiles.length}</span></div>
            {idea.interestedProfiles.length ? <div className="mt-3 divide-y divide-[var(--bc-line)] border-y border-[var(--bc-line)]">{idea.interestedProfiles.map((person) => <Link key={person.userId} href={`/builders/${person.userId}`} className="flex items-center justify-between gap-4 py-4 hover:bg-[var(--bc-surface-subtle)]"><div className="flex items-center gap-3"><Avatar username={person.username} seed={person.userId} size="sm" /><div><p className="text-[13px] font-medium">{person.username}</p><p className="text-[11px] text-[var(--bc-muted)]">{person.role ? ROLE_LABELS[person.role as RoleType] : "Builder"}</p></div></div><span className="text-[12px] font-medium">Zobacz profil →</span></Link>)}</div> : <p className="mt-3 border-y border-[var(--bc-line)] py-6 text-[13px] text-[var(--bc-muted)]">Nikt jeszcze nie zaznaczył zainteresowania. Udostępnij pomysł albo poczekaj na dopasowania.</p>}
          </section>
        ) : null}
      </main>
    </div>
