      ...
      <main className="mx-auto w-full max-w-[1120px]">
        <header className="border-b border-[var(--bc-line-strong)] pb-6">
          ...
          <h1 className="mt-2 max-w-[760px] text-[clamp(30px,4vw,46px)] font-semibold leading-[1.05] tracking-[-0.035em] text-[var(--bc-ink)]">
            {en ? `Your first matches, ${profile.username}.` : `Twoje pierwsze dopasowania, ${profile.username}.`}
          </h1>
          <p className="mt-3 max-w-[700px] text-[14px] leading-6 text-[var(--bc-muted)]">
            {en ? "We start with people and projects that fit your role, availability, interests and goals. You can update these details later in your profile." : "Zaczynamy od ludzi i projektów pasujących do Twojej roli, dostępności, zainteresowań i celów. Te dane możesz później zmienić w profilu."}
          </p>
        </header>

        <section className="mt-8">
          <SectionHeader title={en ? "People worth meeting" : "Ludzie, których warto poznać"} meta={en ? `${builderMatches.length} first matches` : `${builderMatches.length} pierwsze dopasowania`} href="/builders" locale={locale} />
          ...
          ) : <EmptyLine text={en ? "There aren’t enough active profiles yet for a meaningful match." : "Nie ma jeszcze wystarczająco wielu aktywnych profili, aby przygotować sensowne dopasowania."} />}
        </section>
              ...
              {projectMatches.map(({ project, match }) => (
                <article key={project.id} className="grid gap-4 py-5 lg:grid-cols-[minmax(0,1fr)_260px_96px_130px] lg:items-center">
                  ...
                  <div className="border-t border-[var(--bc-line)] pt-4 lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0">
                    ...
                    <p className="mt-1.5 text-[13px] leading-5 text-[var(--bc-muted)]">{match.reasons[0] || (en ? "The project has open roles and is worth checking." : "Ten projekt ma otwarte role i warto go sprawdzić.")}</p>
        ...
        <footer className="mt-10 flex flex-col gap-3 border-t border-[var(--bc-line-strong)] pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="max-w-[560px] text-[13px] leading-5 text-[var(--bc-muted)]">{en ? "This is only the first ranking. As more real activity appears in BuildCrew, the next recommendations will get better." : "To dopiero pierwszy ranking. Wraz z większą aktywnością w BuildCrew rekomendacje będą coraz lepsze."}</p>
          <div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/projects">{en ? "Explore projects" : "Odkryj projekty"}</Link></Button><Button asChild><Link href={nextPath}>{nextPath === "/dashboard" ? (en ? "Go to Home" : "Przejdź do Startu") : (en ? "Continue" : "Kontynuuj")} <ArrowRight className="h-4 w-4" /></Link></Button></div>
        </footer>
      </main>
    </div>
  );
...
function scoreProject(profile: Awaited<ReturnType<typeof getProfileByUserId>>, project: Awaited<ReturnType<typeof listProjects>>[number], locale: AppLocale) {
  ...
  if (exactRoles.length) { score += 45; reasons.push(en ? `The project is looking for ${profile.role ? labels.roles[profile.role as RoleType] : "a role similar to yours"}` : `Projekt szuka osoby w roli: ${profile.role ? labels.roles[profile.role as RoleType] : "podobnej do Twojej"}`); }
  ...
  if (sharedSkills.length) { score += Math.min(30, sharedSkills.length * 10); reasons.push(en ? `Your stack fits: ${sharedSkills.slice(0, 3).join(" · ")}` : `Pasujące umiejętności: ${sharedSkills.slice(0, 3).join(" · ")}`); }
