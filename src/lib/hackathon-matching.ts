...
export function computeHackathonMatch(me: MatchPerson, candidate: MatchPerson, locale: AppLocale = "pl") {
  ...
  if (me.role !== candidate.role) {
    ...
    reasons.push(copy(`${roleLabels[candidate.role]} dobrze uzupełnia Twoją rolę: ${roleLabels[me.role]}.`, `${roleLabels[candidate.role]} complements your ${roleLabels[me.role]} role.`));
  } else {
    score += 4;
  }
  ...
  if (sharedThemes.length) {
    ...
    reasons.push(copy(`Wspólne obszary: ${sharedThemes.slice(0, 2).join(" · ")}.`, `Shared themes: ${sharedThemes.slice(0, 2).join(" · ")}.`));
  }

  const sharedTech = overlap(me.technologies, candidate.technologies);
  if (sharedTech.length) {
    score += Math.min(12, sharedTech.length * 4);
    reasons.push(copy(`Wspólne technologie: ${sharedTech.slice(0, 2).join(" · ")}.`, `Shared stack: ${sharedTech.slice(0, 2).join(" · ")}.`));
  }
  ...
  if (me.availability === candidate.availability) {
    score += 10;
    reasons.push(copy("Macie podobną dostępność podczas wydarzenia.", "You have similar availability during the event."));
