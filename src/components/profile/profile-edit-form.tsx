    ...
    <div className="flex flex-col">
      <FormSection title={copy("Podstawy", "Basics")}>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <Field label={copy("Headline", "Headline")}><Input value={form.headline} onChange={(e) => setForm((f) => ({ ...f, headline: e.target.value }))} placeholder={copy("np. Full-stack developer budujący SaaS-y", "e.g. Full-stack developer building SaaS products")} maxLength={100} /></Field>
      ...
      <FormSection title={copy("Umiejętności", "Skills")}>
        <div className="flex max-h-[28rem] flex-col gap-4 overflow-y-auto pr-1">
          {Object.entries(SKILL_GROUPS).map(([group, skills]) => (
            <div key={group}>
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--bc-faint)]">{copy(group, group === "Integracje" ? "Integrations" : group)}</p>
      ...
      <FormSection title={copy("Dopasowanie", "Matching")}>
        <div>
        ...
        <div className="mt-5"><p className="mb-2 text-sm font-medium">{copy("Obszary", "Interests")}</p><div className="flex flex-wrap gap-1.5">{INTEREST_OPTIONS.map((interest) => <TagButton key={interest} active={form.interests.includes(interest)} onClick={() => setForm((f) => ({ ...f, interests: toggleValue(f.interests, interest) }))}>{interest}</TagButton>)}</div></div>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          ...
          <div><p className="mb-2 text-sm font-medium">{copy("Szukam teraz", "Looking for")}</p><div className="space-y-2">{LOOKING_FOR_OPTIONS.map((value) => <CheckRow key={value} checked={form.lookingFor.includes(value)} label={labels.lookingFor[value]} onChange={() => setForm((f) => ({ ...f, lookingFor: toggleValue(f.lookingFor, value) }))} />)}</div></div>
        </div>
      </FormSection>
      ...
      <FormSection title={copy("Widoczność profilu", "Profile visibility")} hint={copy("Publiczny profil jest opcjonalny i możesz go wyłączyć w dowolnym momencie.", "Your public profile is optional and can be disabled at any time.")}>
        <label className="flex cursor-pointer items-start justify-between gap-5 border-y border-[var(--bc-line)] py-3.5">
          <span className="min-w-0">
            <span className="block text-sm font-medium text-[var(--bc-ink)]">{copy("Publiczny profil buildera", "Public builder profile")}</span>
            <span className="mt-0.5 block max-w-[680px] text-[12px] leading-4 text-[var(--bc-faint)]">{copy("Pozwala udostępniać profil poza BuildCrew i pojawiać się w publicznym portfolio współpracy. Discord i prywatne dane kontaktowe nadal nie są publiczne.", "Lets you share your profile outside BuildCrew and appear in public collaboration history. Discord and private contact details remain private.")}</span>
          </span>
