      ...
      {navItems.map((item) => {
        ...
        return (
          <Link key={item.href} href={item.href} className={cn("relative flex min-w-[68px] flex-1 flex-col items-center gap-1 overflow-hidden px-1 py-1 text-[10px] font-medium transition-colors sm:min-w-0 sm:text-[11px]", active ? "text-[var(--bc-ink)]" : "text-[var(--bc-faint)]")}>
            ...
            {item.href === "/messages" && unreadMessages > 0 ? <span className="absolute right-[24%] top-1 flex min-w-4 items-center justify-center rounded-full bg-[var(--bc-accent)] px-1 text-[11px] font-semibold text-neutral-950">{unreadMessages > 9 ? "9+" : unreadMessages}</span> : null}
          </Link>
        );
      })}
    </nav>
