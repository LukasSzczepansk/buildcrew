...
import { CircleHelp, ExternalLink, FolderKanban, LayoutDashboard, LogOut, MessageCircle, ShieldCheck, Trophy, UserRoundCheck, Users, Waves } from "lucide-react";
...
function NavGroup({ label, items, pathname, unreadMessages }: { label: string; items: typeof PRIMARY_NAV; pathname: string; unreadMessages: number }) {
  return (
    <div>
      <p className="mb-2 px-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--bc-faint)]">{label}</p>
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "group relative flex h-10 items-center gap-2.5 rounded-[6px] px-3 text-[13px] font-medium transition-colors",
            ...
            >
              ...
              {item.href === "/messages" && unreadMessages > 0 ? <span className="min-w-5 rounded-full bg-[var(--bc-accent)] px-1.5 py-0.5 text-center text-[10px] font-semibold text-neutral-950">{unreadMessages > 99 ? "99+" : unreadMessages}</span> : null}
            </Link>
          );
        })}
      </nav>
  ...
  return (
    <aside className="sticky top-0 hidden h-dvh w-[224px] shrink-0 flex-col border-r border-[var(--bc-line)] bg-[var(--bc-surface-subtle)] px-4 py-5 lg:flex">
      ...
      <div className="space-y-7">
        <NavGroup label="Nawigacja" items={PRIMARY_NAV} pathname={pathname} unreadMessages={unreadMessages} />
      ...
      {admin ? (
        <div className="mt-7 border-t border-[var(--bc-line)] pt-4">
          <Link href="/admin" className={cn("flex h-10 items-center gap-2.5 rounded-[6px] px-3 text-[13px] font-medium transition-colors", pathname.startsWith("/admin") ? "bg-neutral-950 text-white dark:bg-white dark:text-neutral-950" : "text-[var(--bc-muted)] hover:bg-[var(--bc-surface)] hover:text-[var(--bc-ink)]")}>
      ...
      {isAiContestActive() ? (
        <a href={DISCORD_INVITE_URL} target="_blank" rel="noopener noreferrer" className="mb-4 rounded-[8px] border border-[var(--bc-line)] bg-[var(--bc-surface)] p-3 text-[11px] leading-4 text-[var(--bc-muted)] hover:text-[var(--bc-ink)]">
      ...
      <div className="mb-4 flex items-center gap-4 px-2 text-[12px] text-[var(--bc-faint)]">
      ...
      <div className="border-t border-[var(--bc-line)] pt-3">
        <Link href="/profile" className="flex items-center gap-3 rounded-[8px] px-2 py-2.5 transition-colors hover:bg-[var(--bc-surface)]">
          <Avatar username={username} seed={avatarEmoji || username} size="sm" className="h-8 w-8 text-[12px]" />
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-medium text-[var(--bc-ink)]">{username}</p>
            <p className="text-[11px] text-[var(--bc-faint)]">Profil i ustawienia</p>
          </div>
        </Link>
