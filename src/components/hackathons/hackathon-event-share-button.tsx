...
export function HackathonEventShareButton({ shortPath }: { shortPath: string }) {
  ...
  return <Button type="button" size="sm" variant="ghost" onClick={copyLink}>{copied ? <Check className="h-3.5 w-3.5" /> : <Link2 className="h-3.5 w-3.5" />}{copied ? copyText("Link skopiowany", "Link copied") : copyText("Copy short link", "Copy short link")}</Button>;
}
