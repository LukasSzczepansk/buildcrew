  ...
  return (
    <div className={compact ? "grid gap-4 sm:grid-cols-2" : "grid gap-5 md:grid-cols-2"}>
      {items.map((item) => {
        ...
        return (
          <article key={item.id} className="min-w-0 overflow-hidden rounded-[10px] border border-[var(--bc-line)] bg-[var(--bc-surface)] shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
            {cover ? (
            ...
            <div className="p-3.5">
              ...
              {item.tools.length ? <div className="mt-3 flex flex-wrap gap-1.5">{item.tools.slice(0, 8).map((tool) => <span key={tool} className="rounded-[5px] border border-[var(--bc-line)] bg-[var(--bc-surface-subtle)] px-2 py-1 text-[10px] font-medium text-[var(--bc-muted)]">{tool}</span>)}</div> : null}
