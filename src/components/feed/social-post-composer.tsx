...
import { BookOpenText, FlagTriangleRight, FolderSearch, Handshake, Lightbulb, Megaphone, MessageCircleQuestion, Plus, Rocket, UserRoundSearch, X } from "lucide-react";
...
export function SocialPostComposer({
  ...
  const [kind, setKind] = React.useState<SocialPostKind>("QUESTION");
  ...
  const options: { kind: SocialPostKind; label: string; icon: React.ReactNode }[] = [
    { kind: "QUESTION", label: copy("Pytanie", "Question"), icon: <MessageCircleQuestion className="h-3.5 w-3.5" /> },
    { kind: "KNOWLEDGE", label: copy("Wiedza / doświadczenie", "Knowledge"), icon: <BookOpenText className="h-3.5 w-3.5" /> },
    { kind: "IDEA", label: copy("Pomysł", "Idea"), icon: <Lightbulb className="h-3.5 w-3.5" /> },
    { kind: "LOOKING_FOR_PEOPLE", label: copy("Szukam ludzi", "Looking for people"), icon: <UserRoundSearch className="h-3.5 w-3.5" /> },
    { kind: "LOOKING_FOR_PROJECT", label: copy("Szukam projektu", "Looking for a project"), icon: <FolderSearch className="h-3.5 w-3.5" /> },
    { kind: "MILESTONE", label: copy("Kamień milowy", "Milestone"), icon: <FlagTriangleRight className="h-3.5 w-3.5" /> },
    { kind: "LAUNCH", label: copy("Premiera", "Launch"), icon: <Rocket className="h-3.5 w-3.5" /> },
    { kind: "OPEN_TO_BUILDING", label: copy("Otwarty/a na współpracę", "Open to building"), icon: <Handshake className="h-3.5 w-3.5" /> },
  ];
  ...
  function submit() {
    startTransition(async () => {
      ...
      toast.success(copy("Opublikowano w społeczności.", "Published to the community."));
      setBody("");
      setExpanded(false);
  ...
  const placeholder = (() => {
    switch (kind) {
      case "QUESTION": return copy("O co chcesz zapytać innych builderów? Dodaj kontekst, żeby łatwiej było Ci pomóc.", "What do you want to ask other builders? Add context so people can actually help.");
      case "KNOWLEDGE": return copy("Czego się nauczyłeś/aś? Opisz konkretny problem, rozwiązanie albo wniosek z budowania.", "What did you learn? Share a concrete problem, solution or lesson from building.");
      case "IDEA": return copy("Opisz pomysł krótko: jaki problem rozwiązuje, dla kogo i czego chcesz się dowiedzieć od społeczności.", "Describe the idea briefly: the problem, who it is for and what feedback you want from the community.");
  ...
  return (
    <section className="overflow-hidden rounded-[12px] border border-[var(--bc-line)] bg-[var(--bc-surface)]">
      <button
        type="button"
        onClick={() => { setKind("QUESTION"); setExpanded(true); }}
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate text-[13px] text-[var(--bc-muted)] sm:text-[14px]">{copy("Czym chcesz się podzielić? Pytaniem, wiedzą, pomysłem…", "What do you want to share? A question, knowledge, an idea…")}</span>
        ...
        <div className="border-t border-[var(--bc-line)] px-4 py-4 sm:px-5">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {projects.length && kind !== "UPDATE" ? <Button type="button" size="sm" variant="ghost" onClick={() => setKind("UPDATE")}><Megaphone className="h-3.5 w-3.5" />{copy("Aktualizacja projektu", "Project update")}</Button> : null}
