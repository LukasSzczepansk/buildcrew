  ...
  return (
    <form onSubmit={submit} className="grid gap-5 lg:grid-cols-[1fr_320px]">
      <Card className="space-y-5 p-6">
        ...
        <label className="flex items-start gap-3 rounded-xl border border-violet-200 bg-violet-50/60 p-4 dark:border-violet-500/20 dark:bg-violet-500/5">
      ...
      <div className="space-y-5">
        <Card className="space-y-4 p-5">
          <div><Label>Powiąż z projektem BuildCrew</Label><select className="mt-1.5 h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm dark:border-neutral-700 dark:bg-neutral-900" value={form.projectId} onChange={(e) => setForm({ ...form, projectId: e.target.value })}><option value="">Projekt solo / bez powiązania</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></div>
          <div><Label>Build Challenge</Label><select className="mt-1.5 h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm dark:border-neutral-700 dark:bg-neutral-900" value={form.challengeId} onChange={(e) => setForm({ ...form, challengeId: e.target.value })}><option value="">Brak</option>{challenges.map((challenge) => <option key={challenge.id} value={challenge.id}>{challenge.title}</option>)}</select></div>
          <div><Label>Kategoria</Label><select className="mt-1.5 h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm dark:border-neutral-700 dark:bg-neutral-900" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as ShowcaseCategory })}>{Object.entries(SHOWCASE_CATEGORY_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
          <div><Label>Status</Label><select className="mt-1.5 h-10 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm dark:border-neutral-700 dark:bg-neutral-900" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as ShowcaseStatus })}>{Object.entries(SHOWCASE_STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
        </Card>
