...
export async function createAnswer(input: z.infer<typeof answerSchema>) {
  ...
  if (question.authorId !== user.id) await createNotification(question.authorId, "QUESTION_ANSWERED", "Ktoś odpowiedział na Twoje pytanie", question.title, `/help/${question.id}`);
...
export async function markAnswerHelpful(answerId: string, questionId: string) {
  ...
  if (answer.authorId !== user.id) await createNotification(answer.authorId, "ANSWER_MARKED_HELPFUL", "Twoja odpowiedź została oznaczona jako pomocna!", question.title, `/help/${questionId}`);
